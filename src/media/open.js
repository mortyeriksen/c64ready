...
export function createOpenMedia(port) {
  ...
  return async function openMedia(request) {
    ...
    try {
      ...
      const disk = validateMedia(data, mediaType);
      ...
      if (action !== 'save') {
        ...
        if (mediaType === 'd64') {
          ...
          await port[mediaType](data, name, { autorun });
          message = { prg: 'PRG loaded', crt: 'Cartridge started', tap: 'Tape loaded', reu: 'Loaded into RAM Expansion' }[mediaType];
        }
      }
      let saved = false;
