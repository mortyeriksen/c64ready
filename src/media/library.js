...
//
// Whenever the user loads a .prg / .d64 / .crt / .tap / .t64 / .sid from disk (file picker
...
//   META_STORE   { id, type, name, size, savedAt, source?, releaseTitle?, provenance? }
//                small records listed in bulk
 ...
 */
export async function libSave(type, name, data, metadata = {}) {
  ...
  const meta = { ...sourceMetadata(existing), ...sourceMetadata(metadata), id, type, name, size: data.length, savedAt: Date.now() };
...
export async function libExport() {
  ...
  for (const m of metas) {
    ...
    out.entries.push({ ...sourceMetadata(e), type: e.type, name: e.name, size: e.size, savedAt: e.savedAt, data: _b64encode(e.data) });
  }
  return out;
}
...
export async function libImport(input, onProgress) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    const ok = await libSave(type, name, data, sourceMetadata(e));
