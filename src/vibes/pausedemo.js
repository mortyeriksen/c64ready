...
//
// Timeline (loops forever, 59 s) — each sweeps in from the left and out to
...
//   Act 4  a classic neon wireframe globe
//   Act 5  a classic neon wireframe torus
//   Act 6  a dark-red solid morphing between cube and sphere
...
// the cubes' surface colour. f=0.7 ≈ 30% darker.
const darken = (hex, f) => {
...
const CUBE_DIM = 0.7;   // surface albedo: 30% darker for all solid cubes
...
const ACTS = [
  ...
  { dur: 9 },   // 3: wireframe globe
  { dur: 9 },   // 4: wireframe torus
  { dur: 12 },  // 5: dark-red solid morphing cube → sphere → cube
];
...
export class PauseDemo {
  ...
  _init() {
    ...
    // Act 3 has its own rig on a dedicated layer: a shadow-casting key light
  ...
  _buildActs() {
    ...
    // Act 4 — a globe: square-celled wireframe over solid black faces.
    {
    ...
    // Act 5 — a torus, same solid-vector treatment.
    {
    ...
    // Act 6 — a dark-red solid that morphs cube → sphere → cube. Uses the
  ...
  _animate(dt) {
    ...
    this._setFade(this.actGlobe,  idx === 3 ? fade : 0);
    this._setFade(this.actTorus,  idx === 4 ? fade : 0);
    this._setFade(this.actMorph,  idx === 5 ? fade : 0);
